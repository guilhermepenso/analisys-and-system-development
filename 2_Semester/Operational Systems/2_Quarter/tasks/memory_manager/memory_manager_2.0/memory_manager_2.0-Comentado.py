...
class GerenciadorMemoria:
    ...
    def alocar(self):
        ...
        # Se encontrar um espaço livre, o processo é alocado nesse espaço, salvando na variável best_fit
        if len(blocos_livres) >= n:
            if best_fit is None or len(best_fit) > len(blocos_livres):
                best_fit = list(blocos_livres)
        ...
        # Aloca na memória usando as posições da variável best_fit com o nome, cor e também altera o status do bloco para "1", ou ocupado
        for k in range(n):
    ...
    def desalocar(self):
        ...
        # Busca o processo pelo nome do processo declarado pelo usuário, altera seus dados para "0", muda o texto e a cor para branco
        for i in range(100):
    ...
    # Busca todos os processos que não estão livres e altera eles para branco e sem 
