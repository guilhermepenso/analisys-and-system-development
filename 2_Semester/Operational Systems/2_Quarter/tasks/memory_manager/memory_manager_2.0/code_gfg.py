...
class free:
	def __init__(self):
		self.tag=-1     # tag do bloco livre
		self.size=0     # tamanho do bloco livre
		self.next=None    # Passo para o próximo bloco livre
free_head = None; prev_free = None    # Declara a cabeça do bloco livre e a anterior como None

# Structure for allocated list
class alloc:
	def __init__(self):
		self.block_id=-1        # ID da alocação é -1
		self.tag=-1       # Tag da alocação é -1    
		self.size=0       # Tamanho da alocação é 0
		self.next=None      # Próxima cabeça como None

alloc_head = None;prev_alloc = None     # Declara a cabeça do bloco alocado e a anterior como None
